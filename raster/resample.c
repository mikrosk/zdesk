...
static void HorizontalFilter( MFDB *source, MFDB *destination, double x_factor, ContributionInfo *contribution, APPVar *app)
{
	long start, stop;
	...
	for( x = 0; x < destination->fd_w; x++)
	{
    	...
    	for( y = 0; y < destination->fd_h; y++)
    	{
			...
			for( i = 0; i < n; i++)
      		{
        		app->raster.getPixel( contribution[i].pixel, y, source_line_size, &source_red, &source_green, &source_blue, source->fd_addr);
      		...
      		app->raster.setPixel( x, y, dst_line_size, RoundToQuantum( gamma * red), RoundToQuantum( gamma * green), RoundToQuantum( gamma * blue), destination->fd_addr);
    	}
    }
}
...
static void VerticalFilter( MFDB *source, MFDB *destination, double y_factor, ContributionInfo *contribution, APPVar *app)
{
    	...
    	for ( x = 0; x < destination->fd_w; x++)
       	{
    		...
    		for ( i = 0; i < n; i++)
       		{
        		app->raster.getPixel( x, contribution[i].pixel, source_line_size, &source_red, &source_green, &source_blue, source->fd_addr);
...
int16 smooth_resize( MFDB *source_image, MFDB *resized_image, APPVar *app)
{
	MFDB tmp_img;
    ...
    algo		 = filters[app->pref.smooth_thumbnail].function;
    algo_support = filters[app->pref.smooth_thumbnail].support;
	...
	if (( resized_image->fd_w * (( uint32) source_image->fd_h + resized_image->fd_h)) > ( resized_image->fd_h * ((uint32) source_image->fd_w + resized_image->fd_w)))
    {
		if( !init_mfdb( &tmp_img, resized_image->fd_w, source_image->fd_h, app->nplanes))
        {
      	...
      	HorizontalFilter( source_image , &tmp_img, x_factor, contribution, app);
     	VerticalFilter( &tmp_img, resized_image, y_factor, contribution, app);
    }
	else
    {
		if( !init_mfdb( &tmp_img, source_image->fd_w, resized_image->fd_h, app->nplanes))
        {
      	...
      	VerticalFilter( source_image, &tmp_img, y_factor, contribution, app);
      	HorizontalFilter( &tmp_img, resized_image, x_factor, contribution, app);
    }
