...
inline void draw_tabbar( WINDOW *win, GRECT *tabbar_area, GRECT *clip, APPVar *app)
{
	...
	pxy[0] = tabbar_area->g_x;
	pxy[1] = tabbar_area->g_y;	
	pxy[3] = pxy[1];
	
	if( tab->tab_xpos >= 0)
	{
		pxy[2] = pxy[0] + tab->tab_xpos + 1;
		pxy[4] = pxy[2];
		pxy[5] = pxy[1] + tabbar_area->g_h - 1;
		pxy[6] = pxy[2] + tab->tab_width - 3;
		pxy[7] = pxy[5];
		pxy[8] = pxy[6];
		pxy[9] = pxy[1];
		pxy[10] = tabbar_area->g_x + tabbar_area->g_w - 1;
		pxy[11] = pxy[1];	
		v_pline( app->vdi_handle, 6, pxy);		
	}
	else 
	{	
		pxy[2] = pxy[0] + tabbar_area->g_w - 1;
	...
	{
		// static uint16 arrow_bitmap[9] = { 0x39c0, 0x1ce0, 0x0e70, 0x0738, 0x039c, 0x0738, 0x0e70, 0x1ce0, 0x39c0}; 
		static uint16 arrow_bitmap[9] = { 0x0200, 0x0300, 0x0F80, 0x0FC0, 0x0FE0, 0x0FC0, 0x0F80, 0x0300, 0x0200}; /* 16 * 9 * 1 bit */
	...
	else if( win->selected_tab->tab_width == MAXIMAL_TAB_WIDTH)
	{
		...
		pxy[0] = pxy[2] + 3;
		pxy[1] = tabbar_area->g_y + 1;
		pxy[2] = pxy[0] + tabbar_area->g_w;
		pxy[3] = pxy[1] + tabbar_area->g_h - 2;
		vr_recfl( app->vdi_handle, pxy);
	}
}
